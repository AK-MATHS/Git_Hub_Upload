...
double FEM<dim>::basis_function(unsigned int node, double xi){
  ...
  /*You can use the function "xi_at_node" (defined above) to get the value of xi (in the bi-unit domain)
    ...
    for (unsigned int i{1}; i < basisFunctionOrder + 2; i++) {
  ...
  /*You can use the function "xi_at_node" (defined above) to get the value of xi (in the bi-unit domain)
    ...
    for(unsigned int A=0; A<dofs_per_elem; A++){
        for(unsigned int q=0; q<quadRule; q++){
            value += (basis_function(A, quad_points[q]) * quad_weight[q]) ;
...
void FEM<dim>::assemble_system(){
  ...
  for (;elem!=endc; ++elem){
	...
	//EDIT - Define Flocal.
          ...
          Flocal[A] += (Area * h_e / 2) * f * x * basis_function(A, quad_points[q]) * quad_weight[q] ;
      }
	...
	//EDIT - Modify Flocal to include the traction on the right boundary.
          Flocal[dofs_per_elem - 1] =+ math.pow(10,6);
      }
    }
	...
	for(unsigned int q=0; q<quadRule; q++){
	  //EDIT - Define Klocal.
        Klocal[A][B] += (2 * E * Area / h_e) * basis_gradient(A, quad_points[q]) * basis_gradient(B, quad_points[q]);
